...
2. **小程序配置**
   - **重要**: 微信小程序需要在 `manifest.json` 中配置 appid
   - 需要配置服务器域名白名单
   - 示例配置：
   ```json
   "mp-weixin": {
...
5. **图片资源**
   - 图片路径使用相对路径或绝对路径
   - 网络图片需要配置合法域名
   - 参考 `ASSETS_CHECKLIST.md` 准备必要的图标资源
...
1. **立即购买**: 当前点击"立即购买"会提示功能开发中，建议先使用"加入购物车"
2. **订单结算**: 购物车结算流程待完善，需要实现订单确认页面
3. **支付功能**: 暂未集成支付接口，需要根据实际需求接入微信支付/支付宝等
